...
contract DumboBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view override returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == DUM ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( DUM ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}
